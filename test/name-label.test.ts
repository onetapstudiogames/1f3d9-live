...
test('short names stay still and retain a thing kind only when both fit', () => {
  assert.deepEqual(labelContent('Small lantern', 'tool', 72, 21), { showKind: true, scroll: false })
  assert.deepEqual(labelContent('Small lantern', 'tool', 108, 21), { showKind: false, scroll: false })
