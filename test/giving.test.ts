...
test('reads only verified typed thing transfers with complete references, and keeps the mode', () => {
  ...
  for (const detail of [
    ...
    // No sale mode is published, so nothing invents one; city issue onetapstudiogames/1f3d9#281.
    { mode: 'sale', asset_type: 'thing', asset_id: 7, resident_id: 3, place_id: 4 },
    { asset_type: 'thing', asset_id: 7, price: 2, resident_id: 3, place_id: 4 },
...
test('live transfer fixture keeps old incomplete rows unlinked and reads verified complete rows', () => {
  ...
  assert.ok(parsed.some(value => value?.thingId === 2122 && value.partnerId === 274 && value.mode === 'gift'))
  assert.ok(parsed.some(value => value?.thingId === 2915 && value.partnerId === 262 && value.mode === 'effect'))
  assert.equal(parsed.some(value => value?.thingId === 23), false)
  // Most of the readable live transfers are effect rows, so the mode carries real weight.
  assert.equal(parsed.filter(value => value?.mode === 'gift').length, 5)
  assert.equal(parsed.filter(value => value?.mode === 'effect').length, 34)
})
...
test('finds two stationary visible residents in a committed visible room', () => {
  ...
  const transfer = { thingId: 3, actor: 'mara', mode: 'gift', partnerId: 9, placeId: 2 } as const
