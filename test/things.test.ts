...
test('use glows an already placed matching thing and ignores carried or unknown things', () => {
  ...
  const next = stepThings(initial, [used, carried], 500)
  assert.equal(next.things[11]!.effect?.kind, 'glow')
  assert.equal(next.things[12], undefined)
  assert.deepEqual(next.issues, [])
})

test('effects use fixed live holds with pure frames', () => {
  const initial = createThings(replay({ 'thing:11': { place_id: 1 } }), layout)
  const used = event('action', { action: 'use', status: 'applied', source_thing_id: 11, place_id: 1 })
  const glowing = stepThings(initial, [used], 1_000)
  assert.equal(glowing.things[11]!.effect!.expiresAt - 1_000, 1_600)
  assert.deepEqual(effectFrame(glowing.things[11]!.effect!, 999), { progress: 0, puff: false, glow: false, crumbs: false })
  assert.equal(effectFrame(glowing.things[11]!.effect!, glowing.things[11]!.effect!.expiresAt).progress, 1)
  assert.deepEqual(effectFrame(glowing.things[11]!.effect!, Number.NaN), { progress: 0, puff: false, glow: false, crumbs: false })
  assert.equal(stepThings(glowing, [], glowing.things[11]!.effect!.expiresAt).pending, false)
...
test('the current withdrawal row and older applied consume row show crumbs then remove the thing', () => {
  ...
  const crumbling = stepThings(initial, [withdrawn], 100)
  assert.equal(crumbling.things[11]!.effect?.kind, 'crumbs')
  const gone = stepThings(crumbling, [], crumbling.things[11]!.effect!.expiresAt)
  ...
  assert.equal(stepThings(legacy, [consumed], 100).things[11]!.effect?.kind, 'crumbs')
  assert.equal(stepThings(legacy, [event('action', { action: 'consume', status: 'noop', source_thing_id: 11, place_id: 1 })], 100).things[11]!.effect, null)
})
...
test('typed floor moves use a reserved destination and do not add an effect', () => {
  ...
  const next = stepThings(state, [moved], 100)
