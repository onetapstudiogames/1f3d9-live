...
import { dueSceneEvents, prepareSceneTimeline, recordedRoomCapacity } from './helpers/recorded-scene.ts'
...
const layout = nestedLayout(places, recordedRoomCapacity(replay, census))
...
test('a register-only visitor reserves world capacity, once per resident even without the census', () => {
  assert.equal(recordedRoomCapacity(replay, census)[195], 1)
  assert.equal(recordedRoomCapacity({ ...replay, timeline: [register, register, move(195, 1)] }, [])[195], 1)
})
...
test('a padded registration actor still arrives and reserves world capacity once', () => {
  ...
  const ground = nestedLayout(places, recordedRoomCapacity(data, census))
  const arrival = stepResidents(createResidents(data, census, ground), [padded], 0, 100, ground)
  assert.equal(recordedRoomCapacity(data, census)[195], 1)
...
test('the figure appears only when the registration is due, then holds a fading sparkle before walking', () => {
  ...
  const timeline = prepareSceneTimeline(replay.timeline)
  const before = dueSceneEvents(timeline, 0, Date.parse(joined) - 1)
  ...
  const due = dueSceneEvents(timeline, before.cursor, Date.parse(joined))
...
test('saved null register starts do not manufacture arrivals for residents who joined before the window', () => {
  ...
  const ground = nestedLayout(data.map.places, recordedRoomCapacity(data, people))
