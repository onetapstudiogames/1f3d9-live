...
test('a later walk waits for the meeting to finish and keeps its next note in order', () => {
  let state = stepResidents(initial(), [signature], 0, 0, layout, 120, pairs)
  ...
  state = stepResidents(state, [move, note], 100, 100, layout, 120, pairs)
  ...
  state = stepResidents(state, [], 0, handshakeDuration(120) + 1, layout, 120, pairs)
...
test('a later gift to either held participant waits without losing its record', () => {
  for (const recipient of [1, 2]) {
    ...
    state = stepResidents(state, [gift], 100, 100, layout, 120, pairs)
...
test('a presentation-hidden partner cannot start a meeting with a visible signer', () => {
  const state = stepResidents(initial(), [signature], 0, 0, layout, 120, pairs, resident => resident.id !== 2)
...
test('a note that would place another figure elsewhere waits until the meeting returns', () => {
  ...
  state = stepResidents(state, [note], 100, 100, layout, 120, pairs)
...
test('two simultaneous signatures cannot send different pairs through each other', () => {
  ...
  const next = stepResidents(state, [signature, second], 0, 0, layout, 120, linked)
...
test('opening at now drains an older signature without leaving a meeting or queue behind', () => {
  const settled = settleAtNow({ ...record, timeline: [signature] }, census, layout, 120, pairs)
