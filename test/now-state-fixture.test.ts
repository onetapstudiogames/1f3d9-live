...
test('opening now uses every current census room and leaves no replay motion pending', () => {
  const layout = nestedLayout(day.map.places, roomCapacity(day, census))
  const settled = settleAtNow(day, census, layout)
...
test('a recorded carry cannot overwrite a later floor move or removal when opening now', () => {
  ...
  const layout = nestedLayout(input.map.places, roomCapacity(input, current))
  const moved = settleAtNow(input, current, layout)
  assert.equal(moved.things.things[2727]?.placeId, 760)
  assert.equal(moved.things.things[2727]?.visible, true)
  const removed = settleAtNow({ ...input, timeline: [...carry, { ...later, kind: 'thing_withdrawn', detail: { thing_id: 2727 } }] }, current, layout)
