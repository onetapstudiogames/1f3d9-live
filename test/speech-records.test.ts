...
import { bubbleFor, bubbleShape, typedBubbleFrame } from '../src/speech.ts'
...
test('hard line breaks and empty lines remain visible when no soft wrapping is needed', () => {
  ...
  const frame = typedBubbleFrame(bubble, bubble.expiresAt - 1)
  assert.equal(frame.text, text)
  assert.equal(frame.revealed, text)
  assert.equal(frame.firstLine, 0)
})
...
test('every saved note excerpt survives typing and scrolling with its exact words and cut flag', () => {
  ...
  for (const note of notes) {
    ...
    const characters = [...new Intl.Segmenter(undefined, { granularity: 'grapheme' }).segment(note.line!)]
    const recovered = new Map<number, string>()
    for (let count = 0; count < characters.length; count += 1) {
      const frame = typedBubbleFrame(bubble, bubble.startedAt + count * bubble.charInterval + 0.01)
      const lines = frame.text.split('\n')
      assert.ok(lines.length <= 4)
      lines.forEach((line, index) => recovered.set(frame.firstLine + index, line))
    }
    assert.equal([...recovered.values()].join(''), note.line!.replaceAll('\n', ''), `recorded note ${note.event_id}`)
    const final = typedBubbleFrame(bubble, bubble.expiresAt - 1)
    assert.equal(final.complete, true)
    assert.equal(final.revealed, note.line)
    assert.ok(bubble.expiresAt - bubble.startedAt - characters.length * bubble.charInterval >= 1_500)
  }
})
