...
export function measureReplay(replay: ReplayFile, census: readonly Resident[]): ReplayMetrics {
  ...
  for (let frame = 0; frame < 1_000_000; frame += 1) {
    ...
    if (clock.time >= clock.end && !pending && cursor >= timeline.length) {
      return Object.freeze({ walkCount: walks.size, totalPath: Math.round(totalPath), longestPath: Math.round(longestPath), dayDurationMs: elapsed })
    }
    elapsed += FRAME_MS
    const nextRecordedAt = timeline[cursor]?.time ?? clock.end
    clock = pending
      ? clock
      : Object.freeze({ ...clock, time: Math.min(clock.end, nextRecordedAt) })
    ...
    residents = stepResidents(residents, due.events, FRAME_MS, elapsed, layout)
