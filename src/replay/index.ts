...
const WALK_SHORTEST_MS = 5_600
const WALK_LONGEST_MS = 8_000
const WALK_BASE_DISTANCE = 1_000
const WALK_LOG_STEP_MS = 300
...
export function createClock(start: string, end: string): Clock {
...
export function advanceClock(clock: Clock, deltaMs: number): Clock {
  ...
  return {
    ...clock,
    time: Math.min(clock.end, clock.time + deltaMs),
  }
}
